  ...
  try {
    ...
    for (const t of territories) {
      ...
      if (t.pending_brand && winningBid) {
        ...
        // CAPTURE winner info BEFORE we null it out
        const winnerBrand = t.pending_brand;
        const winnerColor = t.pending_color || '#00F2FE';
        const winnerTagline = t.pending_tagline || 'Territory holder';
        ...
        if (profile.email) {
          try {
            await sendEmail({
              ...
              html: `<h2>Congratulations ${profile.company || winnerBrand}!</h2>
                ...
                Amount due: $${(total/100).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}<br/>
        ...
        await sendEmail({
          ...
          html: `<h2>${epoch.name} Settlement Complete</h2>
            ...
            <table border="1" cellpadding="6">
              ...
              ${invoicesGenerated.map(i => `<tr><td>${i.territory}</td><td>${i.winner}</td><td>${i.invoiceNumber}</td><td>$${(i.total/100).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>`).join('')}
            </table>
